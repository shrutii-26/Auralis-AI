...
import { Mic, MicOff, Upload, AlertTriangle, CheckCircle, Clock, TrendingUp, MessageSquare, Brain, Zap, Square, Radio } from 'lucide-react'
...
const EMOTION_SCORE = {
  anger: 5, frustration: 4, sadness: 3,
  excitement: 2, happiness: 1, neutral: 0,
}
    ...
    <div style={{ background: '#0e1520', border: `1px solid ${accent || '#1c2a3a'}`, borderRadius: 12, padding: '20px 24px' }}>
      {title && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', color: '#64748b', textTransform: 'uppercase' }}>
            {title}
          </span>
        </div>
      )}
...
function Recorder({ onResult, loading, setLoading, sessionId }) {
  const [mode, setMode]           = useState(null)      // null | 'full' | 'chunk'
  ...
  const startRecording = async (selectedMode) => {
    ...
    if (selectedMode === 'full') {
      ...
      mr.onstop = async () => {
        ...
        try { onResult(await sendAudioBlob(blob, sessionId)) }
      ...
      // Chunk mode — restart recorder every CHUNK_INTERVAL ms
      const runChunk = () => {
      ...
      mr.start()
      chunkIntRef.current = setInterval(runChunk, CHUNK_INTERVAL)
    }
    ...
    <Card title="Live Recording" icon={Mic}>
      {!recording ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {/* Full mode button */}
          <button onClick={() => startRecording('full')} style={{
            display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px',
            background: '#080b10', border: '1px solid #1c2a3a', borderRadius: 8,
            color: '#e2e8f0', cursor: 'pointer', fontSize: 13, textAlign: 'left', width: '100%'
          }}>
            <Mic size={16} color="#00d4ff" />
            <div>
              <div style={{ fontWeight: 600 }}>Record then Analyze</div>
              <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>Analyze after you stop recording</div>
            </div>
          </button>

          {/* Chunk mode button */}
          <button onClick={() => startRecording('chunk')} style={{
            display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px',
            background: '#080b10', border: '1px solid #1c2a3a', borderRadius: 8,
            color: '#e2e8f0', cursor: 'pointer', fontSize: 13, textAlign: 'left', width: '100%'
          }}>
            ...
            <div>
              <div style={{ fontWeight: 600 }}>Chunk-based (every 10s)</div>
              <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>Auto-analyzes while you speak</div>
            </div>
          </button>
        </div>
      ) : (
        <div style={{ textAlign: 'center', padding: '8px 0' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 13, color: '#ef4444', fontWeight: 600 }}>
              {mode === 'chunk' ? 'CHUNK MODE' : 'RECORDING'}
            </span>
          </div>

          <div style={{ fontFamily: 'Space Mono, monospace', fontSize: 32, color: '#e2e8f0', marginBottom: 8 }}>
            {fmt(seconds)}
          </div>

          {mode === 'chunk' && (
            <div style={{ fontSize: 12, color: '#64748b', marginBottom: 12 }}>
              Chunks analyzed: {chunkCount} · Next in {10 - (seconds % 10)}s
            </div>
          )}
  ...
  return (
    <Card title="Upload Audio" icon={Upload}>
      <div
        ...
        style={{
          border: `2px dashed ${drag ? '#00d4ff' : '#1c2a3a'}`, borderRadius: 10,
      ...
      >
        ...
        <p style={{ color: fileName ? '#e2e8f0' : '#64748b', fontSize: 13 }}>
          {fileName || 'Drop .wav / .mp3 here or click'}
        </p>
        <input ref={fileRef} type="file" accept=".wav,.mp3,.m4a,.ogg,.flac" style={{ display: 'none' }}
          onChange={(e) => handleFile(e.target.files[0])} />
      </div>
      <button onClick={handleSubmit} disabled={!file || loading} style={{
        width: '100%', padding: '11px 0',
        background: (!file || loading) ? '#1c2a3a' : '#00d4ff',
        color: (!file || loading) ? '#64748b' : '#080b10',
...
function EmotionTimeline({ history }) {
  ...
  const data = history.map((h, i) => ({
    turn: `T${i + 1}`, score: EMOTION_SCORE[h.emotion] ?? 0,
    emotion: h.emotion, risk: Math.round((h.escalation_risk || 0) * 100),
  }))
  return (
    <Card title="Emotion Timeline" icon={TrendingUp}>
      <ResponsiveContainer width="100%" height={140}>
        <LineChart data={data}>
          ...
          <Tooltip
            contentStyle={{ background: '#0e1520', border: '1px solid #1c2a3a', borderRadius: 8, fontSize: 12 }}
  ...
  const handleResult = (data) => {
    ...
    if (data.emotion?.emotion) {
      setHistory(prev => [...prev, {
        ...
        escalation_risk: data.escalation_risk,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          {!result && !loading && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center',
              height: 300, color: '#64748b', fontSize: 14, flexDirection: 'column', gap: 12 }}>
              <Brain size={36} color="#1c2a3a" />
              <span>Record or upload audio to begin analysis</span>
            </div>
          )}
          ...
          {result && !loading && (
            <>
              ...
              <Card title="Transcript" icon={MessageSquare}>
                <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 13, lineHeight: 1.7,
                  color: result.transcript ? '#e2e8f0' : '#64748b' }}>
                ...
                {result.asr_confidence !== null && (
                  <div style={{ marginTop: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#64748b' }}>
                      ...
                      <span style={{ fontFamily: 'Space Mono, monospace' }}>{Math.round((result.asr_confidence || 0) * 100)}%</span>
                    </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                  ...
                  {emotion ? (
                    <>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
                        <span style={{ fontSize: 28, fontWeight: 700, color: eColor,
                      ...
                      <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {[
                          ['Pitch', `${emotion.pitch_mean?.toFixed(0)} Hz`],
                          ['Energy', emotion.energy_mean?.toFixed(4)],
                          ['Speaking Rate', emotion.speaking_rate?.toFixed(2)],
                          ['Confidence', `${Math.round(emotion.confidence * 100)}%`],
                        ].map(([k, v]) => (
                          <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                            <span style={{ color: '#64748b' }}>{k}</span>
                            <span style={{ fontFamily: 'Space Mono, monospace', color: '#e2e8f0' }}>{v}</span>
                          </div>
                        ))}
                      </div>
                      {result.emotion_retry_count > 0 && (
                        <div style={{ marginTop: 12, fontSize: 11, color: '#f97316' }}>
                          ↺ Reflection loop ran {result.emotion_retry_count}x
                        </div>
                      )}
                    </>
                ...
                <Card title="Intent">
                  {intent ? (
                    <>
                      <span style={{ fontSize: 28, fontWeight: 700, color: '#00d4ff',
                        fontFamily: 'Space Mono, monospace', textTransform: 'capitalize' }}>
                        {intent.intent}
                      </span>
                      ...
                      <div style={{ marginTop: 10, fontSize: 12, display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <span style={{ fontFamily: 'Space Mono, monospace' }}>{Math.round(intent.confidence * 100)}%</span>
                      </div>
                    </>
                  ) : <span style={{ color: '#64748b', fontSize: 13 }}>Not analyzed</span>}
                </Card>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                <Card title="Recommended Action">
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 20, fontWeight: 700, fontFamily: 'Space Mono, monospace',
                        color: result.action === 'escalate' ? '#ef4444' : '#22c55e', textTransform: 'uppercase' }}>
                        {result.action}
                      </div>
                  ...
                  {result.response && (
                    <p style={{ marginTop: 16, fontSize: 13, color: '#94a3b8', lineHeight: 1.6,
                      borderTop: '1px solid #1c2a3a', paddingTop: 14 }}>
                      "{result.response}"
                    </p>
                  )}
                </Card>
                ...
                <Card title="Escalation Risk">
                  <div style={{ textAlign: 'center', padding: '8px 0' }}>
                    ...
                    <ConfidenceBar value={result.escalation_risk || 0}
                      color={result.escalation_risk > 0.7 ? '#ef4444' : result.escalation_risk > 0.4 ? '#f97316' : '#22c55e'} />
                    <p style={{ marginTop: 10, fontSize: 12, color: '#64748b' }}>
                      Session turns analyzed: {history.length}
                    </p>
                  </div>
                </Card>
              </div>
              ...
              {result.error && (
                <Card title="Error" accent="#ef444444">
                  <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 12, color: '#ef4444' }}>{result.error}</p>
                </Card>
              )}
            </>
          )}
